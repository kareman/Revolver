...
public class Qwopper {
   ...
   */
  private static final int MAX_RUNS_BETWEEN_RELOAD = Integer.MAX_VALUE; //10;

  private static final int refColor = new Color(63,92,116).getRGB(); //new Color(47,93,116).getRGB(); //0x9dbcd0;
  private static final int refColor2 = new Color(24,34,41).getRGB(); //new Color(20,33,41).getRGB();
  private static final int refColor3 = new Color(254,255,11).getRGB(); //0xffff00
  ...
  private static boolean colorMatches(int ref, int other) {
    boolean m = colorDistance(ref, other) < RGB_TOLERANCE;
    return m;
  }
  ...
  private static boolean matchesBlueBorder(BufferedImage img, int x, int y) {
    ...
    if (!((x > 4) && (y > 4) && (y < img.getHeight() - 4) && (x < img.getWidth() - 12)))
      return false;
    ...
    boolean lightColor = (
            ...
            !colorMatches(img.getRGB(x, y - 4), refColor) &&
            !colorMatches(img.getRGB(x - 4, y - 4), refColor) &&
            !colorMatches(img.getRGB(x + 4, y - 4), refColor) &&
            !colorMatches(img.getRGB(x - 4, y), refColor)*/
    );

    boolean darkColor = (
            colorMatches(img.getRGB(x, y), refColor2) &&
            colorMatches(img.getRGB(x + 4, y), refColor2) &&
            colorMatches(img.getRGB(x + 8, y), refColor2) &&
            colorMatches(img.getRGB(x + 12, y), refColor2)/* &&
            colorMatches(img.getRGB(x, y + 4), refColor2) &&
            !colorMatches(img.getRGB(x, y - 4), refColor2) &&
            !colorMatches(img.getRGB(x - 4, y - 4), refColor2) &&
            !colorMatches(img.getRGB(x + 4, y - 4), refColor2) &&
            !colorMatches(img.getRGB(x - 4, y), refColor2)*/
    );
  ...
  public int[] findRealOrigin() {
    ...
    clickAt(rob, origin[0] + 220, origin[1] + 220);
  ...
  private void stopRunning() {
    ...
    clickAt(rob, origin[0] + 50, origin[1]);
