...
// Courtesy of http://stackoverflow.com/questions/34579957/drawing-class-drawing-straight-lines-instead-of-curved-lines

extension NSBezierPath {
    
    /// Create smooth NSBezierPath using Hermite Spline
    ///
    ...
    ///
    /// - returns:  An initialized `NSBezierPath`, or `nil` if an object could not be created for some reason (e.g. not enough points).
    ...
    convenience init?(hermiteInterpolatedPoints points:[CGPoint], closed: Bool) {
        ...
        for index in 0 ..< numberOfCurves {
            ...
            curveToPoint(endPt, controlPoint1: ctrlPt1, controlPoint2: ctrlPt2)
        }
        
        if closed { closePath() }
    }
    
    /// Create smooth NSBezierPath using Catmull-Rom Splines
    ///
    ...
    convenience init?(catmullRomInterpolatedPoints points:[CGPoint], closed: Bool, alpha: Float) {
        ...
        for index in startIndex ..< endIndex {
            ...
            curveToPoint(point2, controlPoint1: controlPoint1, controlPoint2: controlPoint2)
        }
